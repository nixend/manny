...
public class RouteUtils {
    ...
    public static TagRoute convertTagroutetoStore(TagRouteDTO tagRoute) {
        ...
        List<Tag> tags = convertTagstoStore(tagRoute.getTags());
        store.setTags(tags);
        return store;
    }

    private static List<Tag> convertTagstoStore(List<TagDTO> tags) {
        List<Tag> tagList = new ArrayList();
        for (TagDTO td : tags) {
            tagList.add(new Tag(td.getName(), td.getAddresses()));
        }
        return tagList;
    }

    private static List<TagDTO> convertTagstoDisplay(List<Tag> tags) {
        List<TagDTO> tagList = new ArrayList();
        for (Tag tg : tags) {
            tagList.add(new TagDTO(tg.getName(), 0, tg.getAddresses()));
        }
        return tagList;
    }
    ...
    public static TagRouteDTO convertTagroutetoDisplay(TagRoute tagRoute) {
        ...
        List<TagDTO> tags = convertTagstoDisplay(tagRoute.getTags());
